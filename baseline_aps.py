...
df_train = pd.read_csv("./dataset/aps_failure_training_set2.csv",
                       header=None, dtype=float)
df_train = df_train.sample(frac=1)
df_test = pd.read_csv("./dataset/aps_failure_test_set2.csv",
                      header=None)
...
model = get_model()
file_path = "baseline_cnn_aps.h5"
checkpoint = ModelCheckpoint(file_path, monitor='val_acc', verbose=2,
                             save_best_only=True, mode='max')
early = EarlyStopping(monitor="val_acc", mode="max", patience=5, verbose=2)
redonplat = ReduceLROnPlateau(monitor="val_acc", mode="max", patience=3, verbose=2)
# callbacks_list = [checkpoint, redonplat]  # early
callbacks_list = [checkpoint, early, redonplat]  # early

model.fit(X, Y, epochs=100, verbose=2, callbacks=callbacks_list,
...
# average https://blog.csdn.net/Tobi_Obito/article/details/118853001
